...
class BasicStepTests(BaseStepTests):
    ...
    def test_generate_commands(self):
        ...
        step._generate_commands()

        exp = [(f'cd {self.output_file_path}; tar zcvf '
                'logs-ConvertJob.tgz ConvertJob/logs'),
               (f'cd {self.output_file_path}; tar zcvf '
                'logs-FastQCJob.tgz FastQCJob/logs'),
               (f'cd {self.output_file_path}; tar zcvf '
                'reports-FastQCJob.tgz FastQCJob/fastqc'),
               (f'cd {self.output_file_path}; tar zcvf '
                'logs-GenPrepFileJob.tgz GenPrepFileJob/logs'),
               (f'cd {self.output_file_path}; tar zcvf '
                'prep-files.tgz GenPrepFileJob/PrepFiles'),
               (f'cd {self.output_file_path}; tar zcvf '
                'reports-QCJob.tgz QCJob/NYU_BMS_Melanoma_13059/'
                'fastp_reports_dir'),
               (f'cd {self.output_file_path}; tar zcvf '
                'reports-QCJob.tgz QCJob/Feist_11661/fastp_reports_dir'),
               (f'cd {self.output_file_path}; tar zcvf '
                'reports-QCJob.tgz QCJob/Gerwick_6123/fastp_reports_dir')]

        # how does reports-QCJob.tgz work when there's multiple files? wouldn't
        # the multiple tars overwirte them all?

        self.assertEqual(step.cmds, exp)

        # confirm failed_samples.html file is added when it exists.

        exp.append(f'cd {self.output_file_path}; '
                   'mv failed_samples.html final_results')

        tmp = join(self.output_file_path, 'failed_samples.html')
        with open(tmp, 'w') as f:
            f.write("This is a file")
        ...
        mg_step.generate_commands(fake_client)
